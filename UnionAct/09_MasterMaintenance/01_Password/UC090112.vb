...
Public Class UC090112
    ...
    Private Sub UC090112_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            Dim strSql = String.Format("SELECT daily_pay_master.c_daily_pay_id AS Id" &
                                      ",daily_pay_master.l_name," &
                                      UtDb.DbStrYYYYMMDDtoDateText("daily_pay_master.d_from") & "," &   'ok
                                      UtDb.DbStrYYYYMMDDtoDateText("daily_pay_master.d_to") & "," & 'ok
                                      " daily_pay_master.l_biko,daily_pay_master.d_ins,staf_attribute_latest_view.staf_attribute_l_name FROM daily_pay_master" &    'ok
                                      " LEFT JOIN staf_attribute_latest_view ON daily_pay_master.c_user_id_ins = staf_attribute_latest_view.c_user_id" &
                                      " WHERE (daily_pay_master.d_from<='{0}' AND daily_pay_master.d_to='" & MDMasterCommon.DEFAULT_TO_DATE & "')" &
                                      " OR (daily_pay_master.d_to<>'" & MDMasterCommon.DEFAULT_TO_DATE & "'" &
                                      " AND daily_pay_master.d_from<='{1}' AND '{2}'<=daily_pay_master.d_to) AND daily_pay_master.c_ksh='{3}'" &
                                      " ORDER BY daily_pay_master.c_daily_pay_id,daily_pay_master.d_to", strDate, strDate, strDate, MDLoginInfo.Ksh)
            ...
            table = clsMdb.ExecuteSql("SELECT c_daily_pay_id & '：' & l_name AS id_name" & _
                                      " FROM daily_pay_master WHERE c_ksh='" & MDLoginInfo.Ksh & "'" & _
                                      " GROUP BY c_daily_pay_id & '：' & l_name" & _
                                      " ORDER BY c_daily_pay_id & '：' & l_name")
            '▼値の表示
    ...
    Private Sub btnShowDetail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnShowDetail.Click
        Try
            ...
            uc = pn.Controls(MDConst.SCREEN_ID_UC090113)
    ...
    Private Sub btnSetEnd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSetEnd.Click
        ...
        Try
            ...
            clsMdb.Connect()
            clsMdb.BeginTran()
            result = clsMdb.ExecuteNonQuery(String.Format("UPDATE daily_pay_master SET d_to='{0}',d_ins='{1}',c_user_id_ins='{2}' WHERE c_daily_pay_id='{3}' AND d_from='{4}' AND d_to='99999999' AND c_ksh='{5}'", _
                                              strTo, Date.Today.ToString, MDLoginInfo.UserId, strId, strFrom, MDLoginInfo.Ksh))
            If result <> -1 Then
                result = clsMdb.ExecuteNonQuery(String.Format("UPDATE daily_pay_master_dtl SET d_to='{0}',d_ins='{1}',c_user_id_ins='{2}' WHERE c_daily_pay_id='{3}' and d_from='{4}' and d_to='99999999'", _
                                                  strTo, Date.Today.ToString, MDLoginInfo.UserId, strId, strFrom))
            End If

            If result <> -1 Then
                clsMdb.CommitTran()
                Me.DataGridView1.SelectedRows(0).Cells(3).Value = Format(MDMasterCommon.GetMonthLastDate(Me.cboToDate.Value), "yyyy/MM/dd")
            Else
                clsMdb.RollbackTran()
            End If
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Try
            ...
            clsMdb.Connect()
            clsMdb.BeginTran()
            result = clsMdb.ExecuteNonQuery(String.Format("DELETE FROM daily_pay_master WHERE c_daily_pay_id='{0}' and d_from='{1}' and d_to='{2}' and c_ksh='{3}'", _
                                            strId, strFrom, strTo, MDLoginInfo.Ksh))
            If result <> -1 Then
                result = clsMdb.ExecuteNonQuery(String.Format("DELETE FROM daily_pay_master_dtl WHERE c_daily_pay_id='{0}' and d_from='{1}' and d_to='{2}'", _
                                                strId, strFrom, strTo))
            End If
    ...
    Private Function updateNextMaxTodate(ByVal strId As String, ByVal clsMdb As CLAccessMdb) As Integer
        Dim strSql As String = String.Format("UPDATE daily_pay_master SET d_to = '" & MDMasterCommon.DEFAULT_TO_DATE & "' " & _
                                             "WHERE c_daily_pay_id='{0}' and c_ksh='{1}' AND d_to=(SELECT max(d_to) AS max_d_to FROM daily_pay_master" & _
                                             " WHERE c_daily_pay_id='{2}' and c_ksh='{3}')", strId, MDLoginInfo.Ksh, strId, MDLoginInfo.Ksh)
        log.Debug(strSql)
        Dim result As Integer = clsMdb.ExecuteNonQuery(strSql)

        If result = -1 Then Return -1

        strSql = String.Format("UPDATE daily_pay_master_dtl SET d_to = '" & MDMasterCommon.DEFAULT_TO_DATE & "' " & _
                                             "WHERE c_daily_pay_id='{0}' AND d_to=(SELECT max(d_to) AS max_d_to FROM daily_pay_master_dtl" & _
                                             " WHERE c_daily_pay_id='{1}')", strId, strId)
        log.Debug(strSql)
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Try
            ...
            If Me.lstPayId.Text.Length > 3 Then
                strWhere = String.Format("WHERE daily_pay_master.c_daily_pay_id  LIKE '{0}%'", Me.lstPayId.Text.Substring(0, 3))
            End If
            clsMdb.Connect()
            table = clsMdb.ExecuteSql(String.Format("SELECT daily_pay_master.c_daily_pay_id AS Id" &
                                      ",daily_pay_master.l_name," & UtDb.DbStrYYYYMMDDtoDateText("daily_pay_master.d_from") & "," & UtDb.DbStrYYYYMMDDtoDateText("daily_pay_master.d_to") &
                                      ",daily_pay_master.l_biko,daily_pay_master.d_ins,staf_attribute_latest_view.staf_attribute_l_name FROM daily_pay_master" &
                                      " LEFT JOIN staf_attribute_latest_view ON daily_pay_master.c_user_id_ins = staf_attribute_latest_view.c_user_id" &
                                      " {0} ORDER BY daily_pay_master.c_daily_pay_id,daily_pay_master.d_from ASC", strWhere))
