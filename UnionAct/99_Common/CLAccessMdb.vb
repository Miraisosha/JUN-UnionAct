#Region "NSCLAccessMdb"
'===========================================================================================================
'   ネームスペース：NSCLAccessMdb
'   クラスＩＤ　　：CLAccessMdb
'   クラス名称　　：データベース関連クラス
'   備考  　　　　：
'===========================================================================================================
...
Namespace NSCLAccessMdb
    Public Class CLAccessMdb
...
#Region "関数"
        ...
        Public Sub Connect(Optional ByVal pIntTimeOut As Integer = -1)
            ...
            Try
                ...
                '   接続文字列設定
                '-------------------------------------------------------------------------------
                cs = MDSystemInfo.DbConnectionString
        ...
        ''' <remarks></remarks>
        Public Function ExecuteSql(ByVal pStrSql As String,
                                   Optional ByVal pIntTimeOut As Integer = -1) As DataTable
            log.Debug(pStrSql)                                                  ' ログ出力（SQL文）
            Dim sql As String = pStrSql
            If MDSystemInfo.SQLType = 1 Then
                sql = sql.Replace("Mid(", "SUBSTRING(")
                sql = sql.Replace("CInt(", "CONVERT(int,")
                sql = sql.Replace("CLng(", "CONVERT(int,")
                sql = sql.Replace("CStr(", "CONVERT(nvarchar,")
                sql = sql.Replace("&", "+")
            End If
            Dim dtRet As DataTable = Nothing                                    ' 処理結果データテーブル
            'Try
            dtRet = New DataTable
            _cm = New System.Data.OleDb.OleDbCommand(sql, _cn, _tran)   ' コマンドオブジェクト生成
