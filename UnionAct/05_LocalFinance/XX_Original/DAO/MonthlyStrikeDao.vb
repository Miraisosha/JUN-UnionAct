...
Namespace DAO.FinancialAffairs.WageReduction
    Public Class MonthlyStrikeDao
        Inherits MonthlyBaseDao
        ' Methods
        Public Sub New()
            MyBase.New("pay_strike_cut_monthly")
        End Sub
        ...
        Public Overloads Function GetPrintDetailData(ByVal CompanyCode As String, ByVal TargetYM As String, ByVal TruncPlace As Integer, ByVal CriterionDate As String) As DataTable
            ...
            Dim cmdText As String = String.Concat(New Object() {"SELECT member.c_staf_id AS ", map.GetPhysicalName(0), ", member.l_name AS ", map.GetPhysicalName(1), ", c_branch.l_name AS ", map.GetPhysicalName(2), ", 0 AS ", map.GetPhysicalName(3), ", 0 AS ", map.GetPhysicalName(4), ", 0 AS ", map.GetPhysicalName(5), ", cut.s_cut AS ", map.GetPhysicalName(6), ", cut.s_cover AS ", map.GetPhysicalName(7), ", cut.s_dues AS ", map.GetPhysicalName(8), " FROM (((SELECT s_cut.c_user_id AS c_user_id,IIF(s_cut.s_pay_cut IS NULL, 0, s_cut.s_pay_cut) AS s_cut,IIF(s_cut.cover IS NULL, 0, s_cut.cover) AS s_cover,IIF(s_cut.s_pay_cut IS NULL, 0, s_cut.s_pay_cut) - IIF(s_cut.cover IS NULL, 0, s_cut.cover) AS s_dues FROM (SELECT c_user_id, s_pay_cut," & MDFinanceCommon.Trunc("s_pay_cut", TruncPlace) & " AS cover FROM pay_strike_cut_monthly WHERE TO_CHAR(d_years,'yyyyMM') = :d_years) s_cut ) cut LEFT OUTER JOIN (SELECT A1.* FROM staf_attribute A1, (SELECT c_user_id, MAX(staf_attribute.d_from) AS d_from FROM staf_attribute WHERE d_from <= :d_from AND c_ksh <= :c_ksh GROUP BY c_user_id) B1 WHERE A1.c_user_id = B1.c_user_id AND A1.d_from = B1.d_from ) member ON (cut.c_user_id = member.c_user_id)) LEFT OUTER JOIN (SELECT A2.* FROM area_local_view A2, (SELECT c_constant_seq, MAX(area_local_view.d_from) AS d_from FROM area_local_view WHERE d_from <= :d_from GROUP BY c_constant_seq) B2 WHERE A2.c_constant_seq = B2.c_constant_seq AND A2.d_from = B2.d_from ) c_branch ON (member.k_local = c_branch.c_constant_seq)) ORDER BY member.k_local, RIGHT('0000000000' + member.c_staf_id, 10)"})
            Try
                Dim command As New NpgsqlCommand(cmdText, MyBase.GetNpgsqlConnection)
                MonthlyStrikeDao._logger.Debug(cmdText)
        ...
        Public Const TABLE_NAME As String = "pay_strike_cut_monthly"
    End Class
End Namespace
