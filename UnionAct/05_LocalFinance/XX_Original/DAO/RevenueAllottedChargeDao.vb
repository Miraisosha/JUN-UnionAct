    ...
    Public Class RevenueAllottedChargeDao
        ...
        Public Function GetAllottedChargeData(ByVal strDateForItem As String, ByVal strDateForAllotted As String) As DataSet
            ...
            Try
                ...
                Dim cmdText As String = "select HED.c_expend_item, HED.d_from, DTL.c_expend_item_seq, HED.l_item_name as " & "â»ñ⁄" & ", DTL.l_item_dtl_name as " & "â»ñ⁄ç◊ñ⁄" & ", CASE WHEN ALLOT.d_standard_date IS NULL THEN NULL ELSE CONVERT(DATE, ALLOT.d_standard_date, 112) END as " & "äÓèÄåé" & ", ALLOT.s_unit_price as " & "éxï•íPâø" & ", ALLOT.s_year_month as " & "îNä‘éxï•åéêî" & ", ALLOT.s_member as " & "ëŒè€ëgçáàıêî" & ", ALLOT.s_total as " & "çáåvã‡äz" & " from (select c_expend_item, d_from, l_item_name, s_item_order from expend_item where d_from <= :date_expend_item and d_to >= :date_expend_item ) HED inner join expend_item_dtl DTL on HED.c_expend_item = DTL.c_expend_item and HED.d_from = DTL.d_from left outer join revenue_allotted_charge ALLOT on ALLOT.d_revenue_str = :d_revenue_str and ALLOT.c_expend_item = HED.c_expend_item and ALLOT.c_expend_item_seq = DTL.c_expend_item_seq order by HED.s_item_order, DTL.s_item_dtl_order "
