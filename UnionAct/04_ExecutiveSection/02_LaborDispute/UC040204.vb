...
#Region "変数宣言"
    Private Const SCREEN_ID As String = SCREEN_ID_UC040204
    Private Const SCREEN_NAME As String = SCREEN_NAME_UC040204                          ' 争議行為通告細部画面
    ' log4net
    ...
    Private STRIKE_KIND As String = "02"                                '争議細部
...
#End Region
#Region "イベント"
    Private Sub UC040204_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '-------------------------------------------------------------------------------
        '   各Screen処理
        '-------------------------------------------------------------------------------
    ...
    Private Sub btnSampleText_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSampleText.Click
        ...
        Try
            ...
            strSamKenmei = "全日空乗組Ｂ第 " + strStrikeId + "号 第４項、但し書きに関する件、以下の通り。"
            strSamHonbun = ""
            strSamHonbun = strSamHonbun + "１．全面24時間限ストライキを決行する。" + vbCrLf + vbCrLf
            strSamHonbun = strSamHonbun + "　（１）" + strYYYY + "年" + strMM + "月" + strDD + "日00時から24時までの24時間のストライキである。" + vbCrLf + vbCrLf
            strSamHonbun = strSamHonbun + "　（２）対象は全組合員とする。但し、下記の者を除く。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　１）" + strYYYY + "年" + strMM + "月" + strDD + "日00時(日本時間)に日本国外にいる者。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　２）海外出張中の者、及び海外駐在の者。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　３）長期病欠中の者。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　４）NCA在籍の者、及びNCA現職出向者でNCA勤務日の者。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　５）AKX出向中の者。" + vbCrLf
            strSamHonbun = strSamHonbun + "　　　　６）その他別途通知する者。" + vbCrLf + vbCrLf
            strSamHonbun = strSamHonbun + "２．" + strYYYY1 + "年" + strMM1 + "月" + strDD1 + "日の休日労働(休日移動)は一切拒否する。" + vbCrLf + vbCrLf
            strSamHonbun = strSamHonbun + "３．" + strYYYY + "年" + strMM + "月" + strDD + "日の連続4日目の勤務が予定されている者の、" + vbCrLf
            strSamHonbun = strSamHonbun + "　　" + strYYYY1 + "年" + strMM1 + "月" + strDD1 + "日の乗務・便乗を一切拒否する。" + vbCrLf + vbCrLf
            strSamHonbun = strSamHonbun + "４．B747-400 移動勤務者(大阪駐在)で、" + strYYYY + "年" + strMM + "月" + strDD + "日が稼働予定者" + vbCrLf
            strSamHonbun = strSamHonbun + "　　(大阪→東京)の" + strYYYY + "年" + strMM + "月" + strDD + "日の勤務を一切拒否する。" + vbCrLf
...
#Region "関数"
        ...
        Try
            ...
            Select Case Me.bytStatus
                Case STATUS_DETAIL
                    ...
                    Me.txtKenmei.Visible = True
                    Me.txtHonbun.Visible = True
        ...
        Try
            ...
            Select Case Me.bytTabKind
                Case STATUS_TAB_HONB
                    ...
                    strsql = strsql & "        stli.l_text,stli.c_strike_id,stli.c_basis_strike_id "
                    ...
                    If (dt.Rows(0).Item("c_strike_info") <> "") Then
                ...
                Case STATUS_TAB_TEMP
                    ...
                    strsql = strsql & "        stli.d_strike,stli.l_stand_name,"
                    strsql = strsql & "        stli.d_strike_start,stli.l_subject,"
                    strsql = strsql & "        stli.l_text,stli.c_strike_work_id,stli.c_basis_strike_id"
    ...
    Private Function PrintPreview() As Boolean
        Dim blnRet As Boolean = False                                                       ' 処理結果
        Dim ds As DS0402P3 = Nothing                                                        ' 帳票用データセット
        ...
        Dim drHeader As DS0402P3.dtHeaderRow = Nothing
        ...
        Try
            ...
            drHeader.l_stand_name = txtUser.Text
            ...
            reportObj = New CR0402P3                                                        ' レポートドキュメント生成
