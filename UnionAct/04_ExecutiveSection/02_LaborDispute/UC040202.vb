...
#Region "変数宣言"
    ...
    Private STRIKE_KIND As String = "01"                                '争議行為
...
#Region "イベント"
    Private Sub UC040202_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        '-------------------------------------------------------------------------------
        '   各Screen処理
        '-------------------------------------------------------------------------------
    ...
    Private Sub btnSampleText_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSampleText.Click
        ...
        Try
            ...
            strSamJiken = "全日空乗組発 " + strKumi + " 第 " + strStrikeId + " 号（" + strYYYY + "年 " + strMM + "月 " + strDD + "日付け）、" + vbCrLf + "******** についての争議。"
            strSamDateTime = strYYYY + "年 " + strMM + "月 " + strDD + "日　零時以降、本問題の解決に至るまでの期間。"
            If (Not ChkNull(txtJiken.Text)) Or (Not ChkNull(txtDateTime.Text)) Then
                If CLMsg.Show("GQ0021", "事件", "日時および時間") = DialogResult.Yes Then
                    txtJiken.Text = strSamJiken
                    txtDateTime.Text = strSamDateTime
                End If
            Else
                txtJiken.Text = strSamJiken
                txtDateTime.Text = strSamDateTime
...
#Region "関数"
    ...
    Private Function PrintPreview() As Boolean
        Dim blnRet As Boolean = False                                                       ' 処理結果
        Dim ds As DS0402P1 = Nothing                                                        ' 帳票用データセット
        ...
        Dim drHeader As DS0402P1.dtHeaderRow = Nothing
        ...
        Try
            ...
            drHeader.BeginEdit()
            drHeader.president_name = GetPresidentName()
            drHeader.leader_name = txtUser.Text
            drHeader.l_event = txtJiken.Text
            drHeader.l_term = txtDateTime.Text
            ...
            reportObj = New CR0402P1                                                        ' レポートドキュメント生成

            If (Me.bytStatus = STATUS_DETAIL) Then
                ' プロパティ設定
                fmPrint.ButtonShowType = 3                                                  ' ボタン形式設定（3=>印刷、2=>キャンセル）
            Else
                ' プロパティ設定
                fmPrint.ButtonShowType = 1                                                  ' ボタン形式設定（0=>登録＆印刷 1=>登録のみ 2=>キャンセル 3=>印刷）
            End If
    ...
    Private Function dataCheck() As Boolean
        ...
        Try
            ...
            End If
            '事件 Check Null
            If ChkNull(txtJiken.Text) Then
                CLMsg.Show("GE0006", "事件")
                txtJiken.BackColor = Color.LightPink
                Me.txtJiken.Focus()
                Return False
            End If
            '日時及び時間 Null
            If ChkNull(txtDateTime.Text) Then
                CLMsg.Show("GE0006", "日時及び時間")
                txtDateTime.BackColor = Color.LightPink
            ...
            End If
            '事件+日時及び時間の合計文字Max34*14文字チェック
            tLineCnt = 0
            charCnt = 0
            strArray = Split(txtJiken.Text, vbCrLf)
            lineCnt = UBound(strArray)
            tLineCnt = tLineCnt + lineCnt + 1
            For i = 0 To lineCnt
                charCnt = strArray(i).Length
                If (charCnt > 34) Then
                    CLMsg.Show("GE0093", "事件")
                    txtJiken.BackColor = Color.LightPink
                    Me.txtJiken.Focus()
                    Return False
                End If
            ...
            Next
            strArray = Split(txtDateTime.Text, vbCrLf)
            lineCnt = UBound(strArray)
            tLineCnt = tLineCnt + lineCnt + 1
            For i = 0 To lineCnt
                charCnt = strArray(i).Length
                If (charCnt > 34) Then
                    CLMsg.Show("GE0093", "日時および時間")
                    txtDateTime.BackColor = Color.LightPink
                    Me.txtDateTime.Focus()
                    Return False
                End If
                tCharCnt = tCharCnt + strArray(i).Length
            Next
            If (tCharCnt > 34 * 14) Then
                CLMsg.Show("GE0103", "事件・日時および時間")
                txtJiken.BackColor = Color.LightPink
                txtDateTime.BackColor = Color.LightPink
                Me.txtJiken.Focus()
                Return False
            ...
            Select Case Me.bytStatus
                ...
                Case STATUS_DETAIL
                    ...
                    If (Me.bytTabKind = STATUS_TAB_HONB) Then
                        ...
                        strsql = strsql & "        stli.d_strike_start,stli.l_event,"
                        ...
                        'Me.dtpSougiDate = dt.Rows(0).Item("d_strike_start")
                        Me.txtJiken.Text = NVL(dt.Rows(0).Item("l_event"))
                        Me.txtDateTime.Text = NVL(dt.Rows(0).Item("l_term"))
                        ...
                        '   SQL作成
                        '-------------------------------------------------------------------------------
                        strsql = " select stli.k_strike_info,stli.c_strike_info,"
                        strsql = strsql & "        stli.d_strike,stli.l_stand_name,"
                        strsql = strsql & "        stli.d_strike_start,stli.l_event,"
                        strsql = strsql & "        stli.l_term,stli.c_strike_id,stli.c_basis_strike_id, "
                        strsql = strsql & "        cndt.l_name as k_strike_kind_name "
                        strsql = strsql & " from   strike_list stli,"
                        strsql = strsql & "        constant_dtl cndt "
                        strsql = strsql & " where  stli.c_basis_strike_id = '" & strStrikeId & "'"
                        strsql = strsql & " and    stli.k_strike_kind = cndt.c_constant_seq"
                        strsql = strsql & " and    cndt.c_constant    = 'SOUGI_KIND' "
                        strsql = strsql & " order by stli.c_period_id,CInt(stli.c_strike_info);"    'chk
                        ' SQL実行
                        dt = clsDb.ExecuteSql(strsql)
                        ...
                        If intRetCnt > 0 Then
                            ' 1件以上の処理
                            dgvSougiList.RowCount = intRetCnt                                                        ' 縦総数設定
                            Me.dgvSougiList.SelectionMode = DataGridViewSelectionMode.FullRowSelect                ' 1行選択
                            Me.dgvSougiList.MultiSelect = False                                                    ' 複数選択なし
                            For i = 0 To intRetCnt - 1                                                              ' レコード数分ループ
                                ' データ設定
                                With Me.dgvSougiList.Rows(i).Cells
                                    .Item(0).Value = dt.Rows(i).Item("k_strike_kind_name")
                                    If IsDBNull(dt.Rows(i).Item("c_strike_id")) Then
                                        .Item(1).Value = "*****"
                                    Else
                                        .Item(1).Value = dt.Rows(i).Item("c_strike_id")
                                    End If
                                    strStrike = dt.Rows(i).Item("d_strike")
                                    .Item(2).Value = strStrike.Substring(0, 4) + "/" + strStrike.Substring(4, 2) + "/" + strStrike.Substring(6, 2)
                                    Me.dgvSougiList.Rows(i).Visible = True
                                End With
                            Next
                        End If
                    ...
                    ElseIf (Me.bytTabKind = STATUS_TAB_TEMP) Then
                        ...
                        '-------------------------------------------------------------------------------
                        strsql = ""
                        strsql = strsql & " select stli.k_strike_info," & vbCrLf
                        strsql = strsql & "        stli.d_strike,stli.l_stand_name," & vbCrLf
                        strsql = strsql & "        stli.d_strike_start,stli.l_event," & vbCrLf
                        strsql = strsql & "        stli.l_term,stli.c_strike_work_id" & vbCrLf
                        strsql = strsql & " from   strike_work_list stli" & vbCrLf
                        strsql = strsql & " where  stli.c_strike_work_id = '" & strStrikeId & "'" & vbCrLf
        ...
        Try
            ...
            Select Case Me.bytStatus
                ...
                Case STATUS_EDIT, STATUS_TRANSIT
                    ...
                    Me.txtSougiDate.ReadOnly = True
                    Me.txtJiken.ReadOnly = False
                    Me.txtDateTime.ReadOnly = False
                    Me.txtUser.BackColor = Color.White
                    Me.txtSougiDate.BackColor = Color.White
                    Me.txtJiken.BackColor = Color.White
                    Me.txtDateTime.BackColor = Color.White
                ...
                Case STATUS_INSERT
                    ...
                    Me.txtUser.ReadOnly = False
                    Me.txtSougiDate.ReadOnly = False
                    Me.txtJiken.ReadOnly = False
                    Me.txtDateTime.ReadOnly = False
    ...
    Private Function getSougiID() As String
        ...
        Try
            ...
            strSql = strSql + " where  c_period_id = '" + NSMDInfo.PeriodId + "'"
            strSql = strSql + " group  by c_period_id "
            strSql = strSql + " order by 1 desc" & UtDb.DbOrderOffset & "; "    'ok
        ...
        Try
            ...
            Select Case Me.bytTabKind
                Case STATUS_TAB_HONB
                    ...
                    strSql = strSql + "','"
                    ' 事件
                    strSql = strSql + Me.txtJiken.Text + "','"
                    ' 日時及び期間
                    strSql = strSql + Me.txtDateTime.Text + "','"
                    ' 件名
                    strSql = strSql + "','"
                    ' 本文
                    strSql = strSql + "','"
        ...
        Try
            ...
            Select Case Me.bytTabKind
                Case STATUS_TAB_HONB
                    ...
                    strSql = strSql + "       l_event        = '" + Me.txtJiken.Text + "',"
                    strSql = strSql + "       l_term        = '" + Me.txtDateTime.Text + "',"
